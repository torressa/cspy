...
class TestsBiDirectional(TestingBase):
    ...
    def test_generated(self):
        ...
        alg = BiDirectional(self.G, self.max_res, self.min_res, method="generated")
        alg.run()
        self.check_result(
    ...
    def test_processed(self):
        ...
        alg = BiDirectional(self.G, self.max_res, self.min_res, method="processed")
        alg.run()
        self.check_result(
            alg, self.result_path, self.total_cost, self.consumed_resources
        )
    ...
    def test_unprocessed(self):
        ...
        alg = BiDirectional(self.G, self.max_res, self.min_res, method="unprocessed")
        alg.run()
        self.check_result(
            alg, self.result_path, self.total_cost, self.consumed_resources
        )
    ...
    def test_unprocessed_time_limit(self):
        ...
        alg = BiDirectional(
            self.G, self.max_res, self.min_res, method="unprocessed", time_limit=0.001
        )
        start = time()
        alg.run()
    ...
    def test_unprocessed_threshold(self):
        ...
        alg = BiDirectional(
            self.G, self.max_res, self.min_res, method="unprocessed", threshold=0
        )
        alg.run()
        self.check_result(
            alg, self.result_path, self.total_cost, self.consumed_resources
    ...
    def test_two_cycle_elimination(self):
        "Test two_cycle_elimination parameter"
        alg = BiDirectional(
            self.G, self.max_res, self.min_res, two_cycle_elimination=True
        )
        alg.run()
        self.check_result(
            alg, self.result_path, self.total_cost, self.consumed_resources
    ...
    def test_input_exceptions(self):
        ...
        self.assertRaises(Exception, BiDirectional, self.G, "x", [1, "foo"], "up")
