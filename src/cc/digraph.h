...
class ResourceMap : public lemon::MapBase<LemonArc, double> {
  const LemonGraph::ArcMap<std::vector<double>>& resource_map;
  const int                                      r;

 public:
  double operator[](Key e) const { return -resource_map[e][r]; }

  ResourceMap(
      const LemonGraph::ArcMap<std::vector<double>>& resource_map_in,
      const int                                      r_in)
...
class DiGraph {
 public:
  ...
  ResourceMap getResourceMap(const int& r) const {
    return ResourceMap(*res_map_ptr, r);
  }
