...
bool operator==(
    const spp_no_rc_res_cont& res_cont_1,
...
bool operator==(
    const spp_spptw_res_cont& res_cont_1,
    const spp_spptw_res_cont& res_cont_2) {
  return (
      res_cont_1.cost == res_cont_2.cost && res_cont_1.time == res_cont_2.time);
}
...
int main() {
  ...
  r_c_shortest_paths(
      ...
      std::allocator<
          r_c_shortest_paths_label<SPPRC_Example_Graph, spp_no_rc_res_cont>>(),
  ...
  r_c_shortest_paths(
      ...
      dominance_spptw(),
      std::allocator<
          r_c_shortest_paths_label<SPPRC_Example_Graph, spp_spptw_res_cont>>(),
      default_r_c_shortest_paths_visitor());
  ...
  for (int i = 0; i < static_cast<int>(opt_solutions.size()); ++i) {
    std::cout << "The " << i << "th shortest path from A to E is: ";
    std::cout << std::endl;
    for (int j = static_cast<int>(opt_solutions_spptw[i].size()) - 1; j >= 0;
         --j)
